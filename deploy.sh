...
do
  ...
  if [ -d $repo_$branch ]; then
    echo "repo already cloned. going to pull."
  else
    echo "repo will be cloned."
    git clone git@github.com:kaveh096/$repo.git $repo_$branch
  fi
  ...
  if [ `$curdir/dropbox_uploader.sh list | grep %out% | wc -l` -eq 1 ]; then
    ...
    rm Package/EvcSolver32.* Package/EvcSolver64.* Package/README.md
    "/cygdrive/c/Program Files (x86)/MSBuild/12.0/Bin/MSBuild.exe" /m /target:build /p:Configuration=Release /p:Platform=Win32 /p:NOREG=1 ArcCASPER.sln
    "/cygdrive/c/Program Files (x86)/MSBuild/12.0/Bin/MSBuild.exe" /m /target:build /p:Configuration=Release /p:Platform=x64 /p:NOREG=1 ArcCASPER.sln
    ...
    $curdir/dropbox_uploader.sh upload %out%
  fi
done
cd $curdir
